...
class RideOfferDetailScreenState extends State<RideOfferDetailScreen> {
        ...
        body: SingleChildScrollView(
          ...
          child: Column(
            ...
            children: [
              ...
              const SizedBox(height: 8.0),
              Text(
                'Proposed Back Time: \n${widget.rideOffer.proposedBackTime!
                    .format(context)}',
                style: const TextStyle(fontSize: 16.0),
              ),
              const SizedBox(height: 8.0),
              Text(
                'Availibility: \n${widget.rideOffer.proposedWeekdays.map((
                    i) => weekdays[i]).join(', ')}',
                style: const TextStyle(fontSize: 16.0),
              ),
              const SizedBox(height: 8.0),
              ...
              const SizedBox(height: 8.0),
              Text(
                'Driver Location: \n(${widget.rideOffer.driverLocation
                    .latitude}, ${widget.rideOffer.driverLocation.longitude})',
                style: const TextStyle(fontSize: 16.0),
              ),
              const SizedBox(height: 8.0),
              Text(
                'Price: \n${widget.rideOffer.price == 0.0 ? 'Free' : widget
                    .rideOffer.price}',
                style: const TextStyle(fontSize: 16.0),
              ),
              const SizedBox(height: 8.0),
                              ...
                              onPressed: () {
                                if (status != RequestedOfferStatus.ACCEPTED) {
                                  ...
                                  ScaffoldMessenger.of(context).showSnackBar(
                                    const SnackBar(
                                      content:
                                      Text('Ride request already accepted!'),
                              ...
                              onPressed: () {
                                if (status != RequestedOfferStatus.REJECTED) {
                                  ...
                                  ScaffoldMessenger.of(context).showSnackBar(
                                    const SnackBar(
                                      content:
                                      Text('Ride request already rejected!'),
                                      duration: Duration(seconds: 1),
                                    ),
                                  );
                                }
              ...
              Center(
                child: !isRequesting
                    ? buildRideOfferActions(
                    context, widget.userState, currentUser)
  ...
  Widget buildMyRideOfferActions(BuildContext context, UserState userState,
  ...
  Widget chatButton(UserModel currentUser) {
    return ElevatedButton(
      onPressed: () {
        ...
        currentUser
            .requestChatWithUser(widget.userState, driverUser!)
            .then((chatRoom) =>
        {
          setState(() {
            isRequesting = false;
  ...
  Widget buildOtherRideOfferActions(BuildContext context, UserState userState,
      UserModel? currentUser) {
    ...
    // Safely access the requestedOfferStatus
    final requestedOfferStatus = widget.rideOffer.requestedUserIds[currentUser
        .email];
      ...
      children: [
        ...
        ElevatedButton(
          onPressed: () {
            ...
            currentUser
                .withdrawRequestRide(userState, widget.rideOffer.id)
                .then((err) {
              setState(() {
                isRequesting = false;
              });
              if (err == null) {
                setState(() {
                  // Remove the user's request from the list to update the UI
                  widget.rideOffer.requestedUserIds.remove(currentUser.email);
                });
                ScaffoldMessenger.of(context).showSnackBar(
                  const SnackBar(
      ...
      children: [
        ...
        ElevatedButton(
          onPressed: () {
            ...
            currentUser
                .requestRide(userState, widget.rideOffer)
                .then((err) {
              setState(() {
                isRequesting = false;
              });
              if (err == null) {
                setState(() {
                  // Update the UI to reflect the new pending request status
  ...
  Widget buildRideOfferActions(BuildContext context, UserState userState,
      UserModel? currentUser) {
    // Check if currentUser is null
    if (currentUser == null) {
      return const Center(
